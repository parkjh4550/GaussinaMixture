...
def get_param():
    ...
    for kernel in ['poly', 'rbf']:
        for c in [1,10,100]:
            for degree in [3,4,5,20]:
                for coef0 in [0, 10, 100]:
...
def find_proper_parameter(data, target, num_mixture):
    ...
    for itr in range(len(param)):
        ...
        for i in range(len(data[0])):
            ...
            # C = 1, 10 , 100 ...
            # kernel = 'poly', 'rbf'
            # degree = 3, 4, 5 ... (in case of 'poly')
            # gamma = 'auto', 'scale'
            # coef0 = 0, 10, 100

            #print(param[itr])
            model = SVC(kernel=param[itr][0], C=param[itr][1],  degree=param[itr][2], coef0 = param[itr][3], gamma=param[itr][4], random_state=1)
            train_data, train_target, val_data, val_target = get_valdata(tot_data, tot_target, val_fold=int(i))
            #print(itr, train_target)
    ...
    for itr, p in enumerate(param):
        print('param : ', p)
        print('accuracy : ', total_acc[itr])

    return param[max_num]

def get_test(data, target, param):
    train_data = copy.deepcopy(data[0])
    train_target = copy.deepcopy(target[0])
    ...
    model = model = SVC(kernel=param[0], C=param[1],  degree=param[2], coef0 = param[3], random_state=1)
...
if __name__ == '__main__':
    ...
    best_param = find_proper_parameter(data, target, num_mixture)
    print('selected param : ', best_param)

    test_acc = get_test(data, target, best_param)
